...
import { Request, storeJettonTransferNotification, storeRequest } from '../wrappers/Order';
...
describe('First stage', () => {
    ...
    beforeEach(async () => {
        ...
        const orderCode = await compile('order-func');
        order = blockchain.openContract(Order.createFromConfig(orderInit, orderCode));
    ...
    it('cancelled message', async () => {
        const cancelTransaction = await seller.send(
            {
                to: order.address,
                value: toNano(1),
                body: beginCell().storeUint(1906957434, 32).endCell()
            }
        );
...
describe('Second stage', () => {
    ...
    it('cancelled message -> sender != owner', async () => {
        const cancelTransaction = await deployer.send(
            {
                to: order.address,
                value: toNano(1),
                body: beginCell().storeUint(1906957434, 32).endCell()
    ...
    it('cancelled message -> sender == owner', async () => {
        ...
        console.log(sellJettonWalletOrder.address);
...
describe('total check', () => {
    ...
    beforeEach(async () => {
        ...
        await checkStage(order, seller, request, false);
        let errJettonTransferResult = await seller.send({
            value: toNano(1),
            to: order.address,
            sendMode: 2,
            body: beginCell().endCell()
        ...
        let cancelTransaction = await seller.send(
            {
                to: order.address,
                value: toNano(1),
                body: beginCell().storeUint(1906957434, 32).endCell()
        ...
        let errNotificationBody = beginCell()
            .store(storeJettonTransferNotification({
                    $$type: 'JettonTransferNotification',
                    query_id: 0n,
                    amount: 5n,
        ...
        errJettonTransferResult = await seller.send({
            value: toNano(1),
            to: order.address,
            sendMode: 2,
            body: errNotificationBody
        ...
        let errMinter = blockchain.openContract(
            Minter.createFromConfig(
                {
                    total_supply: 0n,
                    admin_address: seller.address,
        ...
        let errJettonWalletSeller = blockchain.openContract(
            Wallet.createFromConfig({
                    owner_address: seller.address, jetton_master_address: errMinter.address
                },
                await compile('jetton-wallet')
        ...
        let errMinterDeployResult = await errMinter.sendMint(seller.getSender(), { // 0x642b7d07
            value: toNano('1.5'),
            queryID: 10,
            toAddress: seller.address,
            tonAmount: toNano('0.4'),
        ...
        let errJettonWalletOrder = blockchain.openContract(
            Wallet.createFromConfig({
                    owner_address: order.address, jetton_master_address: errMinter.address
                },
                await compile('jetton-wallet')
        ...
        let buyJettonTransferResult = await deployer.send({
            value: toNano(1),
            to: buyJettonWalletDeployer.address,
            sendMode: 2,
            body: buyTransferBody
        ...
        let sellTransferBody = beginCell()
            .store(storeJettonTransfer({
                $$type: 'JettonTransfer',
                query_id: 0n,
                amount: 10n,
        ...
        let sellJettonTransferResult = await deployer.send({
            value: toNano(1),
            to: sellJettonWalletDeployer.address,
            sendMode: 2,
            body: sellTransferBody
        ...
        sellJettonTransferResult = await seller.send({
            value: toNano(1),
            to: sellJettonWalletSeller.address,
            sendMode: 2,
            body: sellTransferBody
        ...
        expect(sellJettonOrderBalance).toEqual(request.amount_sell + 19n);
        expect(sellJettonSellerBalance).toEqual(9999999990n - 9n);
        await checkStage(order, seller, request, true);
        ...
        cancelTransaction = await deployer.send(
            {
                to: order.address,
                value: toNano(1),
                body: beginCell().storeUint(1906957434, 32).endCell()
        ...
        // cancelTransaction = await seller.send(
        //     {
        //         to: order.address,
        //         value: toNano(1),
        //         body: beginCell().storeUint(1906957434, 32).endCell()
        ...
        // sellJettonSellerBalance = (await sellJettonWalletSeller.getJettonData())[0];
        // sellJettonOrderBalance = (await sellJettonWalletOrder.getJettonData())[0];
        //
        // expect(sellJettonSellerBalance).toEqual(10000000000n - 9n);
        // expect(sellJettonOrderBalance).toEqual(0n + 19n);

        errNotificationBody = beginCell()
            .store(storeJettonTransferNotification({
                    $$type: 'JettonTransferNotification',
                    query_id: 0n,
                    amount: 5n,
        ...
        errMinter = blockchain.openContract(
            Minter.createFromConfig(
                {
                    total_supply: 0n,
                    admin_address: seller.address,
        ...
        errJettonWalletSeller = blockchain.openContract(
            Wallet.createFromConfig({
                    owner_address: seller.address, jetton_master_address: errMinter.address
                },
                await compile('jetton-wallet')
        ...
        errMinterDeployResult = await errMinter.sendMint(seller.getSender(), { // 0x642b7d07
            value: toNano('1.5'),
            queryID: 10,
            toAddress: seller.address,
            tonAmount: toNano('0.4'),
        ...
        errJettonWalletOrder = blockchain.openContract(
            Wallet.createFromConfig({
                    owner_address: order.address, jetton_master_address: errMinter.address
                },
                await compile('jetton-wallet')
        ...
        sellJettonTransferResult = await deployer.send({
            value: toNano(1),
            to: sellJettonWalletDeployer.address,
            sendMode: 2,
            body: sellTransferBody
        ...
        //     .endCell();
        //
        // buyJettonTransferResult = await buyer.send({
        //     value: toNano(1),
        //     to: buyJettonWalletBuyer.address,
        //     sendMode: 2,
        //     body: buyTransferBody
        ...
        expect(sellJettonBuyerBalance).toEqual(request.amount_sell);
        expect(buyJettonSellerBalance).toEqual(request.amount_buy);
        expect(buyJettonBuyerBalance).toEqual(9999999995n - 9n);
        expect(sellJettonOrderBalance).toEqual(0n + 24n);
        expect(buyJettonOrderBalance).toEqual(0n + 14n);
    }, 100000000);
