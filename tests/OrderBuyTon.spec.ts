...
describe('Router', () => {
    ...
    let routerBuyTon: SandboxContract<RouterBuyTon>;
    ...
    beforeEach(async () => {
        ...
        routerBuyTon = blockchain.openContract(await RouterBuyTon.fromInit(deployer.address, toNano(0.01), BigInt(Date.now())));

        const routerDeployResult = await deployer.send(
            {
                value: toNano(0.02),
                to: routerBuyTon.address,
                sendMode: 2,
                init: routerBuyTon.init
            }
        );
    ...
    it('main flow', async () => {
        ...
        const orderBuyTon = blockchain.openContract(OrderBuyTon.fromAddress(await routerBuyTon.getCalculateOrder(seller.address, nonce)));

        const sellJettonWalletRouter = blockchain.openContract(
            Wallet.createFromConfig({
                    owner_address: routerBuyTon.address, jetton_master_address: sellJettonMaster
                },
                sellWalletCode
            )
        );
        ...
        const sellTransferBody = beginCell()
            .store(storeJettonTransfer({
                ...
                query_id: 0n,
                amount: 10n,
                destination: routerBuyTon.address,
                response_destination: routerBuyTon.address,
                custom_payload: beginCell().endCell(),
